...
2. **Analyze Agent**
   - Computes fit score (composite of skills match, company signals, role clarity)
   - Flags missing information, asks if research needed to fill gaps
   - Identifies red flags and potential concerns
   - Uses cheaper/faster LLM for scoring

3. **Write Agent**
   - Generates customized resume based on master JSON Resume
   - Creates cover letter drafts (multiple tone options: formal, casual, adapted to company culture)
...
- **Job Output Folders**: One folder per processed job containing all artifacts
...
└── {company}_{title}_{date}/
    ├── metadata.json          # Job data, fit score, processing info
    ├── job_posting.json       # Parsed structured job data
    ├── resume_customized.json # Modified JSON Resume
    ├── resume_customized.tex  # LaTeX version
    ├── resume_ats.pdf         # ATS-optimized plain PDF
    ├── resume_pretty.pdf      # Visually styled PDF
    ├── cover_letter_*.md      # Cover letter options (multiple tones)
    ├── diff.html              # Inline diff view of resume changes
    └── research/              # (When triggered)
        ├── company_notes.md
        ├── contacts.md
        └── interview_prep.md
```
...
- PDF - ATS-optimized (simple formatting for automated screening)
...
- **Company Signals**: Red flags in reviews, funding concerns, turnover indicators
...
- Identifies potential referral paths (manual lookup for MVP)
...
- [ ] Write Agent: Basic resume customization
- [ ] CLI: `job-flow <url>` command
- [ ] Web form: Simple paste interface
- [ ] Job folder output with core artifacts
- [ ] Simple auth (API key protection)

### Defer to Later
- [ ] Analyze Agent with full fit scoring
- [ ] Review Agent
...
1. **Migration Helper**: Convert existing TeX/PDF resume to JSON Resume
...
- Cached in SQLite for fast agent access
- Sync on startup or file change detection

## Job History (Light Tracking)

- Log each processed job: URL, date, generated outputs, fit score
...
- JSON export of all jobs and outputs
- Per-job folder as zip
- All formats available for flexibility

## Notifications

- Email: Summary with link to web interface
- No inline attachments in MVP
...
- LinkedIn network cross-reference for referral discovery
