...
import { SYSTEM_PROMPT, buildUserPrompt, parseResponse } from "./prompt";
import type { Env, JSONResume, CustomizeRequest, CustomizeResponse } from "./types";
...
app.post("/customize", async (c) => {
  ...
  if (!resume) {
    return c.json({ error: "Master resume not found in R2. Upload resume.json first." }, 500);
  }
...
async function getJobText(input: string): Promise<string> {
  ...
  if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
    try {
      ...
      console.log("Jina fetch failed or returned insufficient content, using raw input");
    } catch (e) {
